...
let nombreCliente = document.getElementById('nombre').innerText;
let apellidoCliente = document.getElementById('apellido').innerText;
...
botonCobrar.addEventListener("click",()=>{
	if (ivaCliente == 'Consumidor Final') {
		let dniCliente = document.getElementById('dni').innerText;
		cliente = [nombreCliente,apellidoCliente,dniCliente,ivaCliente];
		completo = [cliente,carrito,totalCarro];
		llamarCobro();
	}else if(ivaCliente == 'Monotributista' || ivaCliente == 'Responsable Inscripto' || ivaCliente == 'Exento'){
		let cuitCliente = document.getElementById('cuit').innerText;
		cliente = [nombreCliente,apellidoCliente,cuitCliente,ivaCliente];
		completo = [cliente,carrito,totalCarro];			
		llamarCobro();
	}		
});